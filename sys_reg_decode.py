...
def reg_bit_field_decode(fields, reg_name, value):
    for field in fields.findall('field'):
        ...
        if (field_description is not None):
            field_description_full_text = ET.tostring(
                field_description, encoding='unicode')
            field_description_text = re.sub(
                '<.*?>', '', field_description_full_text)
        else:
        ...
        print('\x1b[41m', 'bit[', field_msb, ':', field_lsb, ']',
              '-->', bin(bit_field_value), '\x1b[0m')
        # print the field information
        if (field_name is not None and field_name.text is not None):
            print('Field Name:', '\x1b[42m', ''.join(field_name.itertext()), '\x1b[0m')
        ...
        if (field_values_instance is not None):
            for field_value_instance in field_values_instance.findall('field_value_instance'):
                ...
                if bit_field_value == field_value_num:
                    para = field_value_instance.find(
                        'field_value_description/para')
                    if para is not None:
                        para_full_text = ET.tostring(para, encoding='unicode')
                        para_text = re.sub('<.*?>', '', para_full_text)
                    print(field_value_text+':'+para_text)
                    field_value_link = field_value_instance.find(
                        ".//field_value_links_to")
                    if field_value_link is not None:
                        linked_field_id = field_value_link.get(
                            'linked_field_id')
                        linked_field_condition = field_value_link.get(
                            'linked_field_condition')
                        if linked_field_id is not None:
                            linked_feilds = find_fields_by_id(
                                fields, linked_field_id, reg_name, value)
